...
contriesPartner = ['USA', 'China', 'India', 'France', 'Uk', 'Espana', 'Brazil', 'Turkey', 'Japan', 'Germany',
            'South-africa', 'Ireland', 'Belgium', 'Switzerland', 'Congo', 'Argentina', 'Portugal', 'Australia',
            'Canada']

local = random.choice(contries)
fake = Faker(local)

def getInternationalExposure():
    tmp = ''
    x = []
    for i in range(0, len(contriesPartner)):
        x.append('| ' + contriesPartner[i] + ' | ' + str(random.randint(1000, 10000)) + ' $ ' + '| ' + str(random.randint(1990, 2015)) + '|' )
    shuffle(x)
    for txt in x:
        tmp = tmp + txt + '\n'
    return tmp
...
def addCompany(theme):
    ...
    company = company + "| Contry | Investement | Year |" + '\n'
    company = company + "| ------ | ------ | ------ |" + '\n'
    company = company + getInternationalExposure()
    company = company + "### Employees" + '\n\n'
    nbEmployee = random.randint(11, 100)
    company = company + companyName + "are composed of **" + str(nbEmployee) + "** employees" + '\n'
    company = company + "The overall satisfaction rate is " + random.choice(quali) + " **" + str(random.randint(95, 100)) + '%** \n'
